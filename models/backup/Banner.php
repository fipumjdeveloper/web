...
class Banner extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, image, short_description, id_creator, id_language', 'required'),
			array('index, status, id_creator, id_language', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>50),
			array('short_description', 'length', 'max'=>150),
			array('link_to', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_banner, title, image, short_description, link_to, index, status, id_creator, id_language', 'safe', 'on'=>'search'),
		);
	}
